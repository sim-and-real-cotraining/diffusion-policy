...
from diffusion_policy.model.common.losses import MmdLoss
...
class MmdConfig:
    ...
    base_bandwidth: float = None
    mmd_weight: float = 1.0
    embedding_module_name: str = None
    embedding_buffer_batches: int = 1

    # TODO: add parameters for embedding buffer
    

class TrainDiffusionMmdLossWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # Add hook to extract embeddings
        self.embedding_module_name = cfg.training.mmd_config.embedding_module_name
        if self.embedding_module_name is not None:
            found_embedding_module = False
            for name, module in self.model.named_modules():
                # print name and input and output size of module
                if name == self.embedding_module_name:
                    module.register_forward_hook(self.hook_fn)
                    found_embedding_module = True
            if not found_embedding_module:
                raise ValueError(f"Embedding module {self.embedding_module_name} not found in model.")
        ...
        # MMD loss
        self.mmd_weight = cfg.training.mmd_config.mmd_weight
        self.bandwidths = cfg.training.mmd_config.bandwidths
        self.base_bandwidth = cfg.training.mmd_config.base_bandwidth
        ...
        self.embedding = None
        self.embedding_buffer_batches = cfg.training.mmd_config.embedding_buffer_batches

        # Create embedding buffer
        self.real_embedding_buffer = Queue(maxsize=self.embedding_buffer_batches)
        self.sim_embedding_buffer = Queue(maxsize=self.embedding_buffer_batches)
        for _ in range(self.embedding_buffer_batches):
            self.real_embedding_buffer.put_nowait(torch.zeros(0, device=cfg.training.device))
            self.sim_embedding_buffer.put_nowait(torch.zeros(0, device=cfg.training.device))
    
    def hook_fn(self, model, input, output):
        self.embedding = input[0]
    
    def get_embedding(self):
        if self.embedding_module_name is None:
            return self.model.get_latest_obs_embedding()
        else:
    ...
    def run(self):
        ...
        assert self.num_datasets == 2 # Current only support sim & real cotraining
            ...
            for local_epoch_idx in range(resume_epoch, cfg.training.num_epochs):
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        labels = one_hot_encoding.argmax(dim=1)
                        ...
                        self.update_embedding_buffer(
                            embeddings, 
                            labels, 
                            self.real_embedding_buffer, 
                            self.sim_embedding_buffer
                        )

                        # compute loss
                        raw_loss = self.compute_loss(trajectory, noise, pred)
                        mmd_loss = self.compute_mmd_loss(self.real_embedding_buffer, self.sim_embedding_buffer)
                        loss = raw_loss + self.mmd_weight * mmd_loss
                        loss.backward()
                        ...
                        try:
                            mmd_loss_cpu = mmd_loss.item()
                        except:
                            mmd_loss_cpu = mmd_loss # 0 (no samples from one of the buffers)
                        loss_cpu = loss.item()
                        tepoch.set_postfix(loss=loss_cpu, refresh=False)
                        train_losses.append(loss_cpu)
                        step_log = {
                            'train_loss': loss_cpu,
                            'denoiser_loss': raw_loss_cpu,
                            'mmd_loss': mmd_loss_cpu,
                ...
                if (self.epoch % cfg.training.val_every) == 0:
                    with torch.no_grad():
                        ...
                        val_embeddings_per_dataset = [Queue() for _ in range(self.num_datasets)]
                        ...
                        for dataset_idx in range(self.num_datasets):
                            ...
                            with tqdm.tqdm(val_dataloader, desc=f"Dataset {dataset_idx} validation, epoch {self.epoch}", 
                                ...
                                for batch_idx, batch in enumerate(tepoch):
                                    ...
                                    # Update embedding buffer
                                    embeddings = self.get_embedding()
                                    val_embeddings_per_dataset[dataset_idx].put_nowait(embeddings.detach())
                            ...
                            if len(val_losses) > 0:
                                ...
                                step_log[f'val_denoiser_loss_{dataset_idx}'] = val_loss
                        ...
                        mmd_loss = self.compute_mmd_loss(
                            val_embeddings_per_dataset[0], 
                            val_embeddings_per_dataset[1]
                        )
                        
                        step_log['val_denoiser_loss'] = overall_val_loss
                        try:
                            step_log['val_mmd_loss'] = mmd_loss.item()
                        except:
                            step_log['val_mmd_loss'] = mmd_loss # 0 (no samples from one of the buffers)
    ...
    def compute_mmd_loss(self, real_embeddings_buffer, sim_embeddings_buffer):
        real_embeddings = torch.cat(
            [x.detach() if i < len(self.real_embedding_buffer.queue)-1 else x 
             for i, x in enumerate(self.real_embedding_buffer.queue)], dim=0
        )
        sim_embeddings = torch.cat([
            x.detach() if i < len(self.sim_embedding_buffer.queue)-1 else x
            for i, x in enumerate(self.sim_embedding_buffer.queue)], dim=0
        )

        if len(real_embeddings) == 0 or len(sim_embeddings) == 0:
            return 0
        
        return self.mmd_loss(
            real_embeddings.reshape(real_embeddings.shape[0], -1),
            sim_embeddings.reshape(sim_embeddings.shape[0], -1)
        )
    
    def update_embedding_buffer(self, embeddings, labels, real_buffer, sim_buffer):
        # Classify current embeddings
        embed_dim = int(np.prod(embeddings.shape[1:]))
        current_real_embeddings = embeddings[labels == 0]
        current_sim_embeddings = embeddings[labels == 1]
        current_real_embeddings = current_real_embeddings.reshape(current_real_embeddings.shape[0], embed_dim)
        current_sim_embeddings = current_sim_embeddings.reshape(current_sim_embeddings.shape[0], embed_dim)
        
        # Pop off oldest embeddings from buffer
        real_buffer.get_nowait()
        sim_buffer.get_nowait()

        # Add current embeddings to buffer
        real_buffer.put_nowait(current_real_embeddings)
        sim_buffer.put_nowait(current_sim_embeddings)
...
def main(cfg):
    workspace = TrainDiffusionMmdLossWorkspace(cfg)
    workspace.run()
